...
# psission distribution
def poission_chosenuser(chose_list):
...
def mean_chosenuser(chose_list):
    s = []
    for i in chose_list:
        pr = i / args.num_users
        ni = 0
...
if __name__ == '__main__':    
    ...
    # args.set_privacy_budget = range(2, 20, 2)
...
# 
    args.set_num_Chosenusers = poission_chosenuser(chose_list)
    ...
    args.set_num_Chosenusers = mean_chosenuser(chose_list)
    print(args.set_num_Chosenusers)
    localtime_0 = time.asctime(time.localtime(time.time()))
    main(args)
    localtime = time.asctime(time.localtime(time.time()))
