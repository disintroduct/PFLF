...
def Privacy_account(args, threshold_epochs, noise_list, iter):
    ...
    if args.dp_mechanism != 'CRD':
        # 原算法：一次迭代，隐私预算=$\epsilon$
        # noise_scale = np.sqrt(1)*2 * delta_s*np.sqrt(2*q_s*np.log(1/args.delta))/args.privacy_budget
        # 当离线率为1时
        # noise_scale = delta_s*np.sqrt(2*q_s*threshold_epochs*np.log(1/args.delta))/args.privacy_budget
        # 对比试验：一次迭代，隐私预算=$\epsilon/k$
        noise_scale = args.offline_rate * args.epochs * np.sqrt(1)*2 * delta_s*np.sqrt(2*q_s*np.log(1/args.delta))/args.privacy_budget
